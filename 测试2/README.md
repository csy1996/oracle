# 实验2：用户及权限管理
## 第1步：以system登录到pdborcl，创建角色csy_view和用户csy_user，并授权和分配空间：
```sql
...
## 第2步：新用户csy_user连接到pdborcl，创建表mytable和视图myview，插入数据，最后将myview的SELECT对象权限授予hr用户。
...
SQL>exit
```
## 第3步：用户hr连接到pdborcl，查询csy_user授予它的视图myview
